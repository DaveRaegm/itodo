(function() {
    ...
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-gray-600 font-semibold editable-column"
            data-column="${boardId}-done"
            contenteditable="true">DONE</h2>
        <button aria-label="Add task" class="text-gray-500 hover:text-gray-700"
                onclick="openTaskPopup('${boardId}-done')">
            <i class="fas fa-plus"></i>
        </button>
    </div>
    <div class="space-y-4 drop-zone relative" id="${boardId}-done-tasks"></div>
            </section>
        `;
